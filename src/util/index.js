...
function intersect(pt1, pt2, pt3, pt4) {
  ...
  return !(
    ((pt1.x - pt3.x) * (pt4.y - pt3.y) - (pt1.y - pt3.y) * (pt4.x - pt3.x)) *
      ((pt2.x - pt3.x) * (pt4.y - pt3.y) - (pt2.y - pt3.y) * (pt4.x - pt3.x)) >
      0 ||
    ((pt3.x - pt1.x) * (pt2.y - pt1.y) - (pt3.y - pt1.y) * (pt2.x - pt1.x)) *
      ((pt4.x - pt1.x) * (pt2.y - pt1.y) - (pt4.y - pt1.y) * (pt2.x - pt1.x)) >
      0
  );
}
...
function intersectBound(bound) {
  return (
    isNodeInQuad(VIEW_RECT_BOUND[0], bound) ||
    isNodeInQuad(VIEW_RECT_BOUND[1], bound) ||
    isNodeInQuad(VIEW_RECT_BOUND[2], bound) ||
    isNodeInQuad(VIEW_RECT_BOUND[3], bound) ||
...
export function isSquareOutOfScreen(bound, camera) {
  ...
  let pt1 = getCoordInScreen(x1, y1, camera);
  let pt2 = getCoordInScreen(x2, y2, camera);
  let pt3 = getCoordInScreen(x1, y2, camera);
