...
class Node {
    constructor(x = 0, y = 0, z = 0, uuid = 0, special = false) {
        Object.assign(this, { x, y, z, uuid, special })
    }
}
            ...
            if (
                ...
                childPartTypes.map((type) => {
                    this.childParts[type] &&
                        (res = res.concat(
                            traverseSub.call(this.childParts[type])
                        ))
                })
            } else {
...
export class QuadTree {
    constructor(points, speicalPoints) {
        ...
        for (let i = 0; i < points.length; i += 3) {
            this.addNode(
                new Node(points[i], points[i + 1], points[i + 2], this.uuid++)
            )
        }
        this.addNode(
            new Node(
